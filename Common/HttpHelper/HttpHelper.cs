...
namespace CsharpHttpHelper
{
    ...
    public class HttpHelper
    {
            ...
            catch (WebException ex)
            {
                if (ex.Response != null)
                {
                    using (response = (HttpWebResponse)ex.Response)
                    {
                        ...
                        {
                            result.Html = ex.Message +"|"+ex2.Message;
                        }
                    }
                }
                else
        ...
        /// <param name="item">参数类对象</param>
        /// <returns>返回HttpResult类型</returns>
        public HttpResult DownLoadFile(HttpItem item,String filePath)
        {
            //返回参数
            HttpResult result = new HttpResult();
            try
        ...
        private void GetData(HttpItem item, HttpResult result,String filePath)
        {
            if (response == null)
            {
                return;
        ...
        private bool saveDataToFile(String path)
        {
            ...
            try
            {
                using (FileStream _stream = File.Open(path, FileMode.OpenOrCreate))
                {
                    //GZIIP处理
