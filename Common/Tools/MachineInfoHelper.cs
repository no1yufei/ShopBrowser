...
namespace ShopeeChat.Tools
{
    ...
    {
        static private string machinID = null;
        static public string GetMachineID()
        {
            if(null == machinID)
            {
                try
                {
                    machinID = HashHelper.GetMD5(MachineInfo.Instance.CPUInfo + MachineInfo.Instance.BoardInfo);
                }
                catch(Exception ex)
                {
                    machinID = "0000000000000000";
                }
            }
            return machinID;
        }
        public static void  Initilize()
        {
            if(null == machinID)
            {
                ThreadPool.QueueUserWorkItem(new WaitCallback(initMachinID), null);
            }
        }
        static void initMachinID(object state)
        {
            try
            {
                machinID = HashHelper.GetMD5(MachineInfo.Instance.CPUInfo + MachineInfo.Instance.BoardInfo);
            ...
            {
                machinID = Guid.NewGuid().ToString("N");
                MessageBox.Show("当前硬件和网络环境对账号安全有威胁，请慎重选择软件运行环境和网络！" + ex.Message);
                MessageBox.Show("如果确定要使用当前环境，请注意，你的账号和密码信息保存在一个对第三方开放的环境中！");
                machinID = "0000000000000000";
            }
        }
        #region 获得内存信息API
        ...
        public static int GetUsedPercent()
        {
            ...
            return (int)((mi.ullTotalPhys - mi.ullAvailPhys)* 100/ mi.ullTotalPhys);
        }
        /// <summary>
        /// 获得当前可用物理内存大小
        /// </summary>
    ...
    public class MachineInfo
    {
        ...
        public  void SetIPAddress(string[] ip, string[] submask, string[] getway, string[] dns)
        {
        ...
        /// </summary>  
        string[] ips = null;
        public string[] GetExtenalIpAddress(string testUrl = null)
        {
            string str ="";
            try
            {
                string address = testUrl == null ? "http://2000019.ip138.com/" : testUrl;
                str = GetWebStr(address);
                byte[] buffer1 = Encoding.Default.GetBytes(str);
                byte[] buffer2 = Encoding.Convert(Encoding.UTF8, Encoding.Default, buffer1, 0, buffer1.Length);
                str = Encoding.Default.GetString(buffer2, 0, buffer2.Length);
            }
            catch(Exception xe)
            {

            }
            return ParserIPAddress(str);
        }
        public string[] ParserIPAddress(string html)
        {
        ...
        public string BIOSSerialNumber
        {
            get
            {
                try
                {
                    ...
                    Console.WriteLine("sBIOSSerialNumber:" + sBIOSSerialNumber);
                    return sBIOSSerialNumber;
                }
                catch
                {
                    Console.WriteLine("sBIOSSerialNumber:未取得");
                    return "";
                }
            }
        ...
        public string CPUInfo
        {
            get
            {
                return GetCPUManufacturer() + "|" + GetCPUName() + "|" + GetCPUVersion() + "|" + GetCPUID() + "|" + GetCPUSerialNumber();
            }
        ...
        public string OSInfo
        {
            get
            {
                ...
                try
                {
                    str = GetOSVersion() + "|" + GetSystemType();
                }
                catch(Exception xe)
                {
        ...
        public string BoardInfo
        {
            get
            {
                ...
                try
                {
                    str = GetBoardManufacturer() + "|" + GetBoardType() + "|" + GetBoardID();
                }
                catch (Exception xe) { }
                return str;
            }
